...
;
; Build:
;   make build/mandelbrot_v9958.com
;
; Run from CP/M:
;   MANDELBROT_V9958
;
; The program takes over the display, renders a coloured Mandelbrot set into
; the V9958 G6 bitmap, waits for a key, then asks the BIOS to restore its normal
; console display.
;
; Rendering model:
...
; computation practical on a Z80. ESC aborts between computed rows.
	...
	.module mandelbrot_v9958
	.area CODE (ABS)
	.org 0x0100

; ---------------------------------------------------------------------------
; CP/M and Zephyr-80 BIOS interface
; ---------------------------------------------------------------------------
...
;   HL = payload
;   BC = payload length
;   A=00h/FFh requests BIOS display reset/reinitialization.
...
; Present after this many computed rows. The final row is always presented.
PRESENT_ROW_MASK	= 0x07
...
start:
	; Establish a known BIOS/proxy display state, then take over with G6.
	xor a
	call VIDEO_SEND
...
render_row_loop:
	...
	; Progressive presentation in eight-row batches.
	ld a,(sample_row)
	and #PRESENT_ROW_MASK
	call z,video_present
	jr render_row_loop
...
exit_program:
	; Restore the BIOS-owned console mode/font/cursor before returning to CCP.
	xor a
	call VIDEO_SEND
	ld c,#BDOS_EXIT
	jp BDOS
...
; Input: HL=VRAM address 00000h..0D3FFh.
; The G6 bitmap uses only A15..A0; R#14 receives A15..A14.
...
init_g6:
	; Send one register block for R#0..R#11.
	ld hl,#command_buffer
	ld (hl),#OP_REG_BLOCK
	inc hl
	ld (hl),#0x00
	...
	ld (hl),#0x00		; R4
	inc hl
	ld (hl),#0x00		; R5: sprites unused
	inc hl
	ld (hl),#0x00		; R6
	inc hl
	ld (hl),#0x00		; R7: black border
	inc hl
	ld (hl),#0x00		; R8
	inc hl
	ld (hl),#0x88		; R9: 212 lines + interlace
	inc hl
	ld (hl),#0x00		; R10
	inc hl
	ld (hl),#0x00		; R11
