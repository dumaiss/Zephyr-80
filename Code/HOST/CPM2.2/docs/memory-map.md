...
| `9000h-9FFFh` | FAT BDOS backend | 3749 | 347 | Read-only FS2 client, native file manager, FAT BDOS compatibility layer, DPH and DPB. |
...
The last resident asset ends at `D033h`. Cold boot installs all 64 KiB; OS-owned initialized contents may occupy `C000h-DFFFh` outside the reservations below.
...
| `CE10h-D60Fh` | FAT BDOS persistent-state reservation | Fixed bank-7 state; track/sector and synthetic ALV currently use `926` bytes. |
