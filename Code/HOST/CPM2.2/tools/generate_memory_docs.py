...
IMPLEMENTATION_SYMBOLS = [
    ...
    (("CONSOLE_CODE_START",), "Console BIOS implementation start."),
    (("const",), "Console status implementation."),
    (("conin",), "Blocking console input implementation."),
    (("conout",), "Blocking console output implementation."),
    ...
    (("CONSOLE_CODE_END",), "Console BIOS implementation end."),
    ...
    (("NMI_CODE_START",), "NMI support code start."),
    (("NMI_HANDLER",), "Common high-memory NMI handler."),
    (("NMI_CODE_END",), "NMI support code end."),
...
def write_symbol_map(args: argparse.Namespace, symbols: dict[str, int], manifest: dict[str, str]) -> None:
    ...
    lines.extend(
        [
            ...
            symbol_row(symbols, ("CONIN_SOFT_COUNT",), "Input polling soft counter."),
            symbol_row(symbols, ("CONOUT_SOFT_COUNT",), "Output polling soft counter."),
...
def write_memory_map(args: argparse.Namespace, symbols: dict[str, int], manifest: dict[str, str]) -> None:
    ...
    lines = [
        ...
        range_row(span(cbios_base, bios_code_end - 1), "Zephyr BIOS code", "BIOS jump table, boot, console, storage facade, RAM disk backend, NMI support, banking, and launch routines."),
        ...
        range_row(span(runtime_start, runtime_end), "BIOS runtime state", "Current bank, DMA address, console counters, and banking state."),
        ...
        f"| `{span(require_symbol(symbols, 'CONSOLE_CODE_START'), require_symbol(symbols, 'CONSOLE_CODE_END') - 1)}` | Console BIOS implementation. |",
        f"| `{span(require_symbol(symbols, 'STORAGE_STUB_CODE_START'), require_symbol(symbols, 'STORAGE_STUB_CODE_END') - 1)}` | Storage BIOS facade. |",
        f"| `{span(require_symbol(symbols, 'NMI_CODE_START'), require_symbol(symbols, 'NMI_CODE_END') - 1)}` | NMI support and register snapshot display. |",
