...
IMPLEMENTATION_SYMBOLS = [
    ...
    (("ROM_GATE_CODE_START",), "ROM service gate code start."),
    (("ROM_GATE",), "Enters shadow/copy mode on ROM page 4 and calls a ROM service."),
    (("ROM_GATE_CODE_END",), "ROM service gate code end."),
    (("SD_PROBE_CODE_START",), "Drive dispatcher region start (the B:/C: probes are on ROM page 4)."),
    (("stg_seldsk",), "Drive dispatcher: A: to the ROM backend, B:/C: to volume units 0/1."),
    (("SD_PROBE_CODE_END",), "Drive dispatcher region end."),
...
RUNTIME_STATE = [
    ...
    ("CONSOLE_DRIVER", 2),
    ("CONSOLE_CALLER_SP", 2),
    ("rom_gate_caller_sp", 2),
    ("rom_gate_saved_latch", 1),
    ("rom_gate_saved_iff", 1),
    ("rom_gate_service", 1),
    ("SAVED_BANK", 1),
    ("DMA_BANK", 1),
    ("XMOVE_SRC_BANK", 1),
...
COMMON_DRIVER_DECLARATIONS = [
    ("IOC Bulk transport overflow", "IOC_BULK_CODE_START", "IOC_BULK_CODE_END", 1, 1),
    ("USB keyboard IOC polling helper", "HID_INPUT_CODE_START", "HID_INPUT_CODE_END", 2, 2),
    ("IOC Command transport", "IOC_CMD_CODE_START", "IOC_CMD_CODE_END", 3, 3),
    ("SD-card BIOS backend", "SD_STORAGE_CODE_START", "SD_STORAGE_CODE_END", 4, 4),
    ("Drive dispatcher", "SD_PROBE_CODE_START", "SD_PROBE_CODE_END", 5, 5),
    ("ROM service gate", "ROM_GATE_CODE_START", "ROM_GATE_CODE_END", 5, 5),
...
def write_memory_map(
    ...
    lines = [
        ...
        f"| `{span(require_symbol(symbols, 'STACK_PROBE_CODE_START'), require_symbol(symbols, 'STACK_PROBE_CODE_END') - 1)}` | Cold-boot stack probe: paints the stack window, then starts cold boot. |",
